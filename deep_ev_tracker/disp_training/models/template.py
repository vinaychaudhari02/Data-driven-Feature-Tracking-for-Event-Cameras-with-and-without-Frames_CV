...
class Template(LightningModule):
    ...
    def on_validation_epoch_end(self):
        if self.pose_mode is False:
            ...
            with plt.style.context("ggplot"):
                fig = plt.figure()
                # Convert epe_l2_hist to a numpy array of floats
            ...
            # Convert epe_l2_hist to float32 before logging the histogram
            #self.logger.experiment.add_histogram(
            #    "EPE_hist/val", self.epe_l2_hist.astype(np.float32), self.global_step
            #)
